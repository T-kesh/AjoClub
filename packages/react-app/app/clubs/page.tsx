...
export default function ClubsPage() {
  const { data: activeClubIds, isLoading } = useReadContract({
    address: AJO_CLUB_ADDRESS,
    abi: AJO_CLUB_ABI,
    functionName: "getActiveClubs",
  });

  return (
    <main className="max-w-md mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900">Active Clubs</h1>
        <Link href="/create">
      ...
      {!isLoading && (!activeClubIds || activeClubIds.length === 0) && (
        <div className="text-center mt-16">
          <p className="text-gray-400 mb-4">No active clubs yet.</p>
          <Link href="/create">
      ...
      <div className="flex flex-col gap-3">
        {activeClubIds?.map((id) => <ClubSummary key={id.toString()} clubId={id} />)}
      </div>
    </main>
  );
}
